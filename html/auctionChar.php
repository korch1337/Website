	...
	<tr class="yellow">
		...
		<td>Price</td>
	       ...
	       <?php while($row = $aucPlayers->fetch_object()){
			...
			echo '<td>','<form method="POST" action="'.$_SERVER['PHP_SELF'].'">','<input type="submit" onclick="return confirm('Are you sure you want to do that?');" name="'.$row->id.'" value="Buy '.$row->name.'">','</form>','</td>','</tr>';
